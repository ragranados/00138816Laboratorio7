...
Matrix createLocalK(int e,mesh &m){
    ...
    float tau,kappa, lambda, mi, psi, sigma, eta, J, D,Ae;
    
    //Componentes de K
    // [ A+K  G-H ]
    // [  D   0 ]
    Matrix matrixA,matrixK,matrixG,matrixD,matrixH,negativeMatrixH;
    ...
    productRealMatrix(sigma*Ae/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixH);
    ...
    productRealMatrix(-1,matrixH,negativeMatrixH);
    

    //Colocando submatrices en K
    zeroes(K,9);
    ubicarSubMatriz(K,0,5,0,5,sumMatrix(matrixA,matrixK,6,6));
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixG,negativeMatrixH,6,3));
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(SIGMA);
    float f_y = m.getParameter(ETA);
